...
async function signIfSupabasePath(pathOrUrl: string | null) {
  ...
  // Lazy import so this file still tree-shakes on edge without bundling server libs
...
export default async function ThanksPathPage({ params }:{ params: { sid: string } }) {
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-2xl px-6 py-14">
        ...
        <div className="mt-10 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 p-5">
          ...
          <Link href="/checkout?price=price_1SbmGUCCBLLo4EMcl3h2ZHKl" className="mt-4 inline-block rounded-full bg-emerald-500 px-5 py-2 text-sm font-medium text-white">Explore Bundle</Link>
        </div>
