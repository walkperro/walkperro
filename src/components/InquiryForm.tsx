...
// Generalized inquiry form — topic: website | bot | course. Posts to the
// same /api/websites-inquiry endpoint (adds a `topic` field; the API's
// existing validation is topic-agnostic). WebsiteInquiryForm remains as a
// thin wrapper with topic="website", so /websites/[slug] is unchanged.

import { useState } from "react";
import { cn } from "@/lib/utils";

export type InquiryTopic = "website" | "bot" | "course";

type Props = {
  topic?: InquiryTopic;
  templateSlug: string;          // e.g. "restaurant" | "trading-bot" | "guide"
  templateTitle: string;         // display name in copy + email subject
};
...
const TOPIC_COPY: Record<
  InquiryTopic,
  { heading: string; intro: (title: string) => string; projectLabel: string; projectPlaceholder: string }
> = {
  website: {
    heading: "// i want a website like this",
    intro: (t) =>
      `you picked ${t}. tell me about the business you want it for. only email is required — every other field helps me scope it faster.`,
    projectLabel: "TELL ME ABOUT THE BUSINESS",
    projectPlaceholder:
      "what does it do, who's it for, what do you want the site to do for it?",
  },
  bot: {
    heading: "// i want a bot built",
    intro: () =>
      `tell me what you want automated. strategy, market, broker if you have one. only email is required.`,
    projectLabel: "TELL ME ABOUT THE BOT",
    projectPlaceholder:
      "what should it trade or automate? what's the edge you want coded?",
  },
  course: {
    heading: "// i want a course or guide made",
    intro: () =>
      `tell me what you know that's worth teaching. i turn it into a course or guide people pay for. only email is required.`,
    projectLabel: "TELL ME ABOUT THE MATERIAL",
    projectPlaceholder:
      "what's the topic, who's it for, what format do you imagine?",
  },
};
...
export default function InquiryForm({
    ...
    return (
      <div className="border border-charcoal bg-bone p-6 md:p-8">
        ...
        <p className="mt-4 text-charcoal/80">
          your <strong>{templateTitle}</strong> inquiry is in my inbox. check
          yours — i'll reach out from walkperro.com.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <p className="text-charcoal/80 text-sm">{copy.intro(templateTitle)}</p>
      ...
      <div className="flex flex-col gap-2">
        <label className="label" htmlFor="wif-project">// {copy.projectLabel}</label>
        <textarea
          id="wif-project"
          name="project"
          rows={4}
